...
This module writes the freshly-created Odysseus session cookie into the local
Obsidian Codexian plugin settings file. It never reads browser cookies and it
never stores Odysseus passwords.
"""
...
def sync_codexian_odysseus_session(
    *,
    ...
    The login endpoint should not fail just because the local bridge cannot
    update Codexian, so all expected failures are returned as status metadata.
    """

    if os.getenv(CODEXIAN_BRIDGE_ENV, "1").strip().lower() in {"0", "false", "no", "off"}:
    ...
    try:
        ...
        next_odysseus_local.update(
            {
                "enabled": True,
                "baseUrl": existing_base_url.strip()
...
def clear_codexian_odysseus_session(*, data_path: Optional[Path] = None) -> dict[str, Any]:
