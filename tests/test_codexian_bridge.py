...
def _bridge_module(monkeypatch, enabled=True):
    ...
    settings.get_setting = lambda key, default=None: enabled if key == "codexian_cookie_bridge_enabled" else default
...
def test_sync_codexian_odysseus_session_updates_existing_settings(tmp_path, monkeypatch):
...
def test_sync_codexian_odysseus_session_skips_when_already_current(tmp_path, monkeypatch):
    bridge = _bridge_module(monkeypatch)
    data_path = tmp_path / "data.json"
    data_path.write_text(
        json.dumps(
...
def test_sync_codexian_odysseus_session_can_be_disabled_by_env(tmp_path, monkeypatch):
    bridge = _bridge_module(monkeypatch)
    data_path = tmp_path / "data.json"
...
def test_sync_codexian_odysseus_session_can_be_disabled_by_setting(tmp_path, monkeypatch):
    bridge = _bridge_module(monkeypatch, enabled=False)
    data_path = tmp_path / "data.json"
    data_path.write_text("{}", encoding="utf-8")
...
def test_clear_codexian_odysseus_session_removes_cookie(tmp_path, monkeypatch):
    bridge = _bridge_module(monkeypatch)
    data_path = tmp_path / "data.json"
    data_path.write_text(
        json.dumps(
