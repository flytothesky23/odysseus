...
function selectedSummary(total = indexedNotes.length) {
  return `${total.toLocaleString('ko-KR')}개 MD 색인 · ${selectedPaths().length}/${MAX_CHAT_EVIDENCE}개 채팅 근거 선택`;
}

function renderFolder(folder, level, searching) {
  const paths = descendantNotePaths(folder);
  const selection = folderSelectionState(paths, selectedPaths());
  ...
  return `<li class="vault-explorer-folder" role="none">
    <div class="vault-explorer-folder-row" role="treeitem" aria-level="${level}" aria-expanded="${isExpanded}" data-scope-state="${selection.state}" style="--tree-level:${level}">
      ...
      <input type="checkbox" data-folder-select="${uiModule.esc(folder.path)}" aria-label="${uiModule.esc(folder.name)} 폴더 채팅 근거 선택" ${selection.state === 'checked' ? 'checked' : ''}>
    </div>${children}</li>`;
}

function renderNote(note, level) {
  const chosen = selectedPaths().includes(note.path);
  const aliases = Array.isArray(note.aliases) && note.aliases.length ? ` · ${note.aliases.join(', ')}` : '';
  return `<li role="none">
    <label class="vault-explorer-note-row" role="treeitem" aria-level="${level}" data-included="${chosen}" style="--tree-level:${level}" title="${uiModule.esc(`${note.path}${aliases}`)}">
      <span class="vault-explorer-guide" aria-hidden="true"></span>
      <input type="checkbox" data-note-select="${uiModule.esc(note.path)}" aria-label="${uiModule.esc(note.title || note.filename || note.path)} 채팅 근거 선택" ${chosen ? 'checked' : ''}>
...
function renderTree() {
  ...
  const empty = indexedNotes.length
    ? '검색 조건과 일치하는 metadata가 없습니다.'
    : 'Vault를 연결하면 제목·파일명·상대경로·aliases만 먼저 색인합니다.';
  meta.textContent = selectedSummary(indexedNotes.length);
  treeHost.innerHTML = notes.length
    ? `<ul role="tree" aria-label="Obsidian Vault 노트">${tree.folders.map(folder => renderFolder(folder, 1, Boolean(query.trim()))).join('')}${tree.notes.map(note => renderNote(note, 1)).join('')}</ul>`
    : `<div class="vault-explorer-empty">${empty}</div>`;
  treeHost.querySelectorAll('[data-folder-select]').forEach(inputEl => {
    const folder = findFolder(tree, inputEl.dataset.folderSelect);
    const selection = folderSelectionState(descendantNotePaths(folder), selectedPaths());
    inputEl.indeterminate = selection.state === 'mixed';
  });
  const useButton = pane.querySelector('#vault-explorer-use-chat');
  if (useButton) useButton.disabled = selectedPaths().length === 0;
}
...
function applySelection(paths, checked) {
  const state = loadState();
  const result = updateBoundedSelection(state.selected_paths, paths, checked, MAX_CHAT_EVIDENCE);
  saveState({ ...state, selected_paths: result.selected_paths });
  renderTree();
  if (result.rejected_count) {
    setStatus(`채팅 근거는 최대 ${MAX_CHAT_EVIDENCE}개입니다. ${result.rejected_count}개는 선택하지 않았습니다.`, 'warning');
  } else {
    setStatus(`${result.selected_paths.length}/${MAX_CHAT_EVIDENCE}개 노트를 채팅 근거로 선택했습니다.`, 'ok');
  }
}
...
async function indexVault() {
  ...
  saveState({
    ...
    selected_paths: [],
    kordoc_job_ids: loadState().kordoc_job_ids,
    law_job_ids: loadState().law_job_ids,
  });
  ...
  syncRootReadout();
  renderTree();
  setStatus(`${result.note_count}개 metadata 색인 완료 · Vault 본문 읽기 0회`, 'ok');
}
...
function wirePanel() {
  ...
  pane.querySelector('#vault-explorer-refresh').addEventListener('click', () => indexVault().catch(error => setStatus(`${error.code ? `${error.code}: ` : ''}${error.message}`, 'error')));
  pane.querySelector('#vault-explorer-root-select').addEventListener('click', async () => {
    await workspaceModule.openWorkspaceBrowser();
    document.getElementById('workspace-use')?.addEventListener('click', () => {
      syncRootReadout();
      setStatus('Workspace가 바뀌었습니다. Vault 상대경로를 확인한 뒤 색인을 새로고침하세요.');
    }, { once: true });
  });
  ...
  pane.querySelector('#vault-explorer-select-visible').addEventListener('click', () => {
    const paths = filterVaultNotes(indexedNotes, pane.querySelector('#vault-explorer-search').value).map(note => note.path);
    applySelection(paths, true);
  });
  pane.querySelector('#vault-explorer-clear-selection').addEventListener('click', () => applySelection(selectedPaths(), false));
  ...
  pane.querySelector('#vault-explorer-open-workspace').addEventListener('click', () => contractReviewModule.openContractReview());
  ...
  pane.querySelector('#vault-explorer-tree').addEventListener('change', event => {
    ...
    if (noteInput) {
      applySelection([noteInput.dataset.noteSelect], noteInput.checked);
      return;
    }
    ...
    if (folderInput) {
      ...
      applySelection(descendantNotePaths(folder), folderInput.checked);
    }
  });
...
export function openPanel() {
  ...
  pane.innerHTML = `
    ...
    <div class="vault-explorer-rootbar">
      <button type="button" id="vault-explorer-root-select" class="vault-explorer-root-btn">${FOLDER_ICON}<span id="vault-explorer-workspace-label">${uiModule.esc(workspaceLabel())}</span></button>
      <input id="vault-explorer-vault-path" value="${uiModule.esc(state.vault_path || '.')}" aria-label="Workspace 상대 Vault 경로" placeholder="Workspace 상대 Vault 경로" spellcheck="false">
      <button type="button" id="vault-explorer-refresh" class="vault-explorer-icon-btn" title="Metadata index 새로고침" aria-label="Vault metadata 새로고침"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 11a8 8 0 1 0 2 5.3"/><path d="M20 4v7h-7"/></svg></button>
    </div>
    ...
    <div class="vault-explorer-scope-actions">
      <button type="button" id="vault-explorer-select-visible">검색 결과 선택</button>
      <button type="button" id="vault-explorer-clear-selection">전체 해제</button>
      <span>LLM 본문 근거는 최대 ${MAX_CHAT_EVIDENCE}개</span>
    </div>
    <div id="vault-explorer-tree" class="notes-pane-body vault-explorer-tree"></div>
    <p id="vault-explorer-status" class="vault-explorer-status" aria-live="polite">선택은 현재 채팅의 Contract Review 근거 범위에만 적용됩니다.</p>
    <div class="notes-pane-footer vault-explorer-footer">
      ...
      <button type="button" id="vault-explorer-use-chat" class="confirm-btn confirm-btn-primary">선택 근거로 채팅</button>
    </div>`;
